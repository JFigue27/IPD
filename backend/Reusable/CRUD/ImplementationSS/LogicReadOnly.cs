...
namespace Reusable.CRUD.Implementations.SS
{
    public abstract class ReadOnlyLogic<Entity> : BaseLogic, ILogicReadOnly<Entity>, ILogicReadOnlyAsync<Entity> where Entity : class, IEntity, new()
    {
        ...
        virtual public List<Entity> GetAllAdapted()
        {
            ...
            if (cache != null)
            {
                Log.Info($"Cache. Get All Adapted of Type: [{EntityInfo.EntityName}] by User: [{Auth.UserName}].");
            ...
            Log.Info($"SQL. Get All Adapted of Type: [{EntityInfo.EntityName}] by User: [{Auth.UserName}]");
        ...
        virtual public async Task<List<Entity>> GetAllAdaptedAsync()
        {
            ...
            if (cache != null)
            {
                Log.Info($"Cache. Get All Adapted Async of Type: [{EntityInfo.EntityName}] by User: [{Auth.UserName}].");
                return AdapterOut(cache.ToArray());
            }
            ...
            Log.Info($"SQL. Get All Adapted Async of Type: [{EntityInfo.EntityName}] by User: [{Auth.UserName}]");
            if (!CacheDisabled) Cache.Set(CACHE_GET_LIST, entities);
            return AdapterOut(entities.ToArray());
        }
        ...
        virtual public CommonResponse GetPaged(int perPage = 0, int page = 1, string generalFilter = "", SqlExpression<Entity> query = null, string cacheKey = null, bool requiresKeysInJsons = false)
        {
            ...
            if (cache != null)
            {
                Log.Info($"Cache. Get Paged: [{cacheKey}] of Type: [{EntityInfo.EntityName}] by User: [{Auth.UserName}].");
            ...
            //if (cacheFromContainer)
            //{
            Log.Info($"SQL. Get Paged: [{cacheKey}] of Type: [{EntityInfo.EntityName}] by User: [{Auth.UserName}].");
        ...
        virtual public async Task<CommonResponse> GetPagedAsync(int perPage = 0, int page = 1, string generalFilter = "", SqlExpression<Entity> query = null, string cacheKey = null, bool requiresKeysInJsons = false)
        {
            if (string.IsNullOrWhiteSpace(cacheKey)) cacheKey = CACHE_GET_PAGED;
            cacheKey += $"_{perPage}_{page}_{generalFilter}";
            ...
            if (cache != null)
            {
                Log.Info($"Cache. Get Paged Async: [{cacheKey}] of Type: [{EntityInfo.EntityName}] by User: [{Auth.UserName}].");
                var cacheList = cache.Result as IEnumerable<Entity>;
                AdapterOut(cacheList.ToArray());
                return cache;
            }
            ...
            //if (cacheFromContainer)
            //{
            Log.Info($"SQL. Get Paged Async: [{cacheKey}] of Type: [{EntityInfo.EntityName}] by User: [{Auth.UserName}].");
            cacheContainer[cacheKey] = response;
            if (!CacheDisabled) Cache.Replace(CACHE_CONTAINER_GET_PAGED, cacheContainer);
            //}
            return response;
        ...
        virtual public Entity GetSingleWhere(string Property, object Value, SqlExpression<Entity> query = null, string cacheKey = null)
        {
            ...
            if (cache != null)
            {
                Log.Info($"Cache. Get Single Where: [{cacheKey}] of Type: [{EntityInfo.EntityName}] by User: [{Auth.UserName}].");
                return AdapterOut(cache)[0];
            }
            //}
            #endregion
            ...
            //if (cacheFromContainer)
            //{
            Log.Info($"SQL. Get Single Where: [{cacheKey}] of Type: [{EntityInfo.EntityName}] by User: [{Auth.UserName}].");
            cacheContainer[cacheKey] = response;
        ...
        virtual public async Task<Entity> GetSingleWhereAsync(string Property, object Value, SqlExpression<Entity> query = null, string cacheKey = null)
        {
            ...
            if (entity != null)
            {
                Log.Info($"Cache. Get Single Where Async: [{cacheKey}] of Type: [{EntityInfo.EntityName}] by User: [{Auth.UserName}].");
                AdapterOut(entity);
            }
            ...
            //if (cacheFromContainer)
            //{
            Log.Info($"SQL. Get Single Where Async: [{cacheKey}] of Type: [{EntityInfo.EntityName}] by User: [{Auth.UserName}].");
            cacheContainer[cacheKey] = response;
            if (!CacheDisabled) Cache.Replace(CACHE_CONTAINER_GET_SINGLE_WHERE, cacheContainer);
            //}
            return response;
